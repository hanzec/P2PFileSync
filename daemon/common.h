...
class Config {
 public:
  ...
  [[nodiscard]] uint16_t get_manage_port_number() const { return _manage_port_number; }

 private:
  const uint16_t _trans_port_number;
  const uint16_t _manage_port_number;
