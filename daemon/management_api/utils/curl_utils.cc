...
inline bool get_data_from_server(const HttpMethod http_method, const void* input_data,
  ...
  if (curl) {
    ...
    switch (http_method) {
      ...
      case POST:
        ...
        break;
      case DELETE:
        if ((ret = curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "DELETE")) != CURLE_OK)
          goto error;
        break;
    }
...
bool GET_and_save_to_path(CURLSH* curl_handler, const std::string& request_url,
  ...
  // preform downloading
  bool ret = Curl::get_data_from_server(Curl::GET, &file_stream, nullptr, false, curl_handler,
...
void* POST_and_save_to_ptr(CURLSH* curl_handler, const std::string& request_url,
  ...
  if (Curl::get_data_from_server(Curl::POST, data, post_data, is_json, curl_handler,
                                 request_url, {}, write_to_ptr, force_ssl)) {
...
void* GET_and_save_to_ptr(CURLSH* curl_handler, const std::string& request_url,
  ...
  if (Curl::get_data_from_server(Curl::GET, data, nullptr, false, curl_handler, request_url,
                                 header, write_to_ptr, force_ssl)) {
    VLOG(3) << "total of [" << data->head << "] bytes downloaded!";
    auto ret = data->data;
